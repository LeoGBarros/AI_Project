...
## Diagrama ASCII — Fluxo de Refresh Token
...
└────┬─────┘                              └────┬─────┘
     │                                         │
     │  access_token expirado (401 ou exp)     │
     │                                         │
     │  POST /token                            │
     ...
     │                 ▼       ▼               │
     │  ┌──────────────────┐  ┌────────────┐  │
     │  │ Rotaciona token  │  │ 400 error  │  │
     │  │ Emite novo par:  │  │invalid_grant│  │
     │  │ access_token     │  └─────┬──────┘  │
     │  │ refresh_token    │        │          │
     │  └────────┬─────────┘        │          │
     │           │                  │          │
     ...
     │                              │          │
     │◄─────────────────────────────┘          │
     │  Sessão encerrada → login novamente     │
     │                                         │
     │  Repete request com novo access_token   │
     │─────────────────►┌──────┐               │
     │                  │ Kong │               │
     │◄─────────────────┤200 OK│               │
     │                  └──────┘               │
...
> Anterior: [Login do Usuário (ROPC)](auth-ropc-login-flow.md)
> Próximo: [Service-to-Service](auth-client-credentials-s2s.md)
